...
function runTest(conn) {
    ...
    admin.createUser({user: 'userAdmin', pwd: 'Password@a1b', roles: ['userAdminAnyDatabase'], "passwordDigestor" : "server"});
    ...
    (function testCreateUser() {
        ...
        assert.throws(function() {
            db.createUser({user: 'user1', pwd: 'Password@a1b', roles: ['read'], "passwordDigestor" : "server"});
        });
        assert.eq(0, db.getUser('user1').roles.length);

        // Try to create user with role that doesn't exist
        assert.throws(function() {
            db.createUser({user: 'user2', pwd: 'Password@a1b', roles: ['fakeRole'], "passwordDigestor" : "server"});
        });

        // Try to create user with invalid arguments
        assert.throws(function() {
            db.createUser({user: '', pwd: 'Password@a1b', roles: ['read'], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: ['user2'], pwd: 'Password@a1b', roles: ['read'], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
        ...
        assert.throws(function() {
            db.createUser({user: 'user2', pwd: 'Password@a1b', roles: [1], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: 'user2', pwd: 'Password@a1b', roles: [{role: 'read'}], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: 'user2', pwd: 'Password@a1b', roles: [{db: 'test'}], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: 'user2', pwd: 'Password@a1b', roles: [{role: 'read', db: ''}], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: 'user2', pwd: 'Password@a1b', roles: [{role: '', db: 'test'}], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: 'null\u0000char', pwd: 'Password@a1b', roles: [], "passwordDigestor" : "server"});
        });
        assert.throws(function() {
            db.createUser({user: 'null\0char', pwd: 'Password@a1b', roles: [], "passwordDigestor" : "server"});
        });
        // Regression test for SERVER-17125
        assert.throws(function() {
            db.getSiblingDB('$external').createUser({user: '', roles: [], "passwordDigestor" : "server"});
        });
