...
void ServiceEntryPointImpl::startSession(transport::SessionHandle session) {
    ...
    if(serverGlobalParams.externalConfig.getPublicIpPrivateIpRange().isPublicIp(session->remote().host())) {
         session->setPublicIp();
    }

    if(serverGlobalParams.externalConfig.getPrivateIpPrivateIpRange().isPrivateIp(session->remote().host())) {
    ...
    auto ssm = ServiceStateMachine::create(_svcCtx, session, transportMode);
    {
        ...
        if (connectionCount >= internalConnectCount) {
           ...
           warning() << "totalConnectionCount: " << connectionCount << " is smaller than internalConnectCount: " << internalConnectCount;
        ...
        if(!session->inAdminWhiteList()) {
            if (externalConnectCount > _maxNumConnections) {
                if (!quiet) {
                    log() << "connection refused because too many open connections: " << externalConnectCount;
                }
                return;
            }
        }
            ...
            if(internalConnectCount > _maxInternalConnections){
                if (!quiet) {
                    log() << "connection refused because too many internal connections: " << internalConnectCount;
                }
                return;
            }
        }
    ...
    if (!quiet) {
        ...
        log() << "connection accepted from " << session->remote() << " #" << session->id() << " ("
              << connectionCount << word << ", internalCount: " << internalConnectCount << " now open)";
    }
    ...
    ssm->setCleanupHook([ this, ssmIt, session = std::move(session) ] {
        ...
        auto remote = session->remote();
        {
            ...
            if(session->inAdminWhiteList())
            {
                if (_currentInternalConnections.load() >= 1) {
                   ...
                   warning() << "currentInternalConnections: " << _currentInternalConnections.load() << " is smaller than 1";
        ...
        const auto word = (connectionCount == 1 ? " connection"_sd : " connections"_sd);
        log() << "end connection " << remote << " ( connectCount: " << connectionCount << word << ", internalCount: " << internalConnectCount <<" now open)";
