...
bool isAuthWithCustomer(Client* client) {
    return AuthorizationSession::get(client)->isAuthWithCustomer() || 
        (client->isCustomerConnection() && AuthorizationSession::get(client)->isAuthWithCustomerOrNoAuthUser()) ||
...
bool needLogNS(const std::string& inputNs, const std::string& auditOp) {
    ...
    if (serverGlobalParams.auditOpFilterStr == "all" || (serverGlobalParams.auditOpFilter & auditOpFromString(auditOp)) != 0) {
        return true;
    }
    ...
    for (auto& nsMapInConfig : allowSetOpNss) {
        ...
        if ((!nsInConfIsDB && inputNs == nsInConfig)
            ...
            || (nsInConfIsDB && !inputNsIsDB && boost::algorithm::starts_with(inputNs, nsInConfig + "."))) {
            return true;
        }
    }
    return false;
...
void logCommandAuthzCheck(Client* client,
    ...
    if (nullptr != command.getCommand()) {
        ...
        if(std::find(logger::auditOpList.begin(), logger::auditOpList.end(), cmdFilterName) == logger::auditOpList.end()) {
