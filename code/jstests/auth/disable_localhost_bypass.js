...
conn1.getDB("admin").createUser({user: "admin", pwd: "Password@a1b", roles: ["root"], "passwordDigestor" : "server"});
...
assert.throws(function() {
    conn2.getDB("admin").createUser({user: "admin", pwd: "Password@a1b", roles: ["root"], "passwordDigestor" : "server"});
});
