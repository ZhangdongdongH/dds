...
function runAllUserManagementCommandsTests(conn, writeConcern) {
    ...
    conn.getDB('admin').createUser({user: 'admin', pwd: 'Github@12', roles: ['root'], "passwordDigestor" : "server"}, writeConcern);
    ...
    conn.getDB('admin').createUser({
            ...
            roles: [{'role':'readWrite', db: 'test'}, {role: 'testRole', db: 'test'}, {role: 'adminRole', db: 'admin'}], "passwordDigestor" : "server"
    },
                                   writeConcern);
    ...
    userAdminConn.getDB('admin').auth('backupuser', 'Github@12');
    ...
    (function testCreateUser() {
        ...
        testUserAdmin.createUser({user: "andy", pwd: "Github@12", roles: [], "passwordDigestor" : "server"}, writeConcern);

        var user = testUserAdmin2.getUser('monitor');
        assert.eq(10028, user.customData.zipCode);
        assert(db.getSisterDB('admin').auth('monitor', 'Github@12'));
    ...
    (function testUpdateUser() {
        ...
        testUserAdmin.getSisterDB('admin').updateUser('monitor', {pwd: 'Github@124', customData: {}, "passwordDigestor" : "server"}, writeConcern);
        var user = testUserAdmin.getSisterDB('admin').getUser('monitor');
        assert.eq(null, user.customData.zipCode);
        assert(!db.getSisterDB('admin').auth('monitor', 'Github@12'));
        assert(db.getSisterDB('admin').auth('monitor', 'Github@124'));

        testUserAdmin.getSisterDB('admin').updateUser(
            'monitor', {customData: {zipCode: 10036}, roles: [{role: "read", db: 'test'}, {role: "testRole", db: 'test'}]}, writeConcern);
        var user = testUserAdmin.getSisterDB('admin').getUser('monitor');
        ...
        testUserAdmin.getSisterDB('admin').updateUser(
            'monitor', {roles: [{ role: "readWrite", db: 'test'}, {role: 'adminRole', db: 'admin'}]}, writeConcern);
    ...
    (function testUsersInfo() {
        ...
        res = testUserAdmin.getSisterDB('admin').runCommand({usersInfo: {user: 'monitor', db: 'admin'}});
        assert.eq(1, res.users.length);
        assert.eq(10036, res.users[0].customData.zipCode);

        // UsersInfo results are ordered alphabetically by user field then db field,
        // not by user insertion order
        res = testUserAdmin.getSisterDB('admin').runCommand({usersInfo: ['monitor', {user: 'backupuser', db: 'admin'}]});
        printjson(res);
