...
function copydbWithinShardedCluster(useReplSets, passCredentials, useAuth) {
    ...
    if (useAuth) {
        mongos.getDB("admin").createUser({user: "super", pwd: "Password@a1b", roles: ["root"], "passwordDigestor" : "server"});
    ...
    if (passCredentials) {
        var result =
            mongos.getDB('admin').copyDatabase('test1', 'test2', undefined, "super", "Password@a1b");
    } else {
