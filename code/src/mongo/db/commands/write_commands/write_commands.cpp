                ...
                if(_batch.getNamespace().ns() == std::string("admin.system.users")) {
                    for (auto&& doc : _batch.getDocuments()) {
                        ...
                        if (status1.isOK() && status2.isOK()
                            ...
                            CommandHelpers::appendCommandStatusNoThrow(result, 
                                Status(ErrorCodes::Unauthorized, "userName: " + userName + " conflicted with builtin users within admin db."));
                            return;
                        }
                    }
                }
                ...
                if (_batch.getNamespace().ns() == std::string("admin.system.roles")) {
                    for (auto&& doc : _batch.getDocuments()) {
                        ...
                        if (status1.isOK() && status2.isOK()
                            ...
                            CommandHelpers::appendCommandStatusNoThrow(result, 
                                Status(ErrorCodes::Unauthorized, "roleName: " + roleName + " conflicted with builtin roles within admin db."));
                            return;
                        }
                    }
                }
        ...
        if (AuthorizationSession::get(opCtx->getClient())->isAuthWithCustomerOrNoAuthUser() || opCtx->isCustomerTxn()) {
            if(Newbatch.getNamespace().ns() == std::string("admin.system.users")) {
                if(Newbatch.getWriteCommandBase().getUsermanagerCmd() == false) {
                    CommandHelpers::appendCommandStatusNoThrow(result, Status(ErrorCodes::Unauthorized, "unauthorized. Suggest use updateUser cmd."));
                    return;
                } else {
            ...
            if (Newbatch.getNamespace().ns() == std::string("admin.system.roles")) {
                if(Newbatch.getWriteCommandBase().getUsermanagerCmd() == false) {
                    CommandHelpers::appendCommandStatusNoThrow(result, Status(ErrorCodes::Unauthorized, "unauthorized. Suggest use updateRole cmd."));
                    return;
                } else {
...
private:
    ...
    private:
        ...
        if (AuthorizationSession::get(opCtx->getClient())->isAuthWithCustomerOrNoAuthUser() || opCtx->isCustomerTxn()) {
            if(Newbatch.getNamespace().ns() == std::string("admin.system.users")) {
                if(Newbatch.getWriteCommandBase().getUsermanagerCmd() == false) {
                    CommandHelpers::appendCommandStatusNoThrow(result, Status(ErrorCodes::Unauthorized, "unauthorized. Suggest use dropUser cmd."));
                    return;
                } else {
                    std::set<std::string> buildinUsers;
            ...
            if (Newbatch.getNamespace().ns() == std::string("admin.system.roles")) {
                if(Newbatch.getWriteCommandBase().getUsermanagerCmd() == false) {
                    CommandHelpers::appendCommandStatusNoThrow(result, Status(ErrorCodes::Unauthorized, "unauthorized. Suggest use dropRole cmd."));
                    return;
                } else {
                    std::set<std::string> buildinRoles;
                    RoleName::getBuildinRoles(buildinRoles);
