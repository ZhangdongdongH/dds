        ...
        if(opCtx->getClient()->isFromPrivateIp1()) {
            ...
            if(isMasterResponse.hasHosts()) {
                ...
                for(auto& hp : isMasterResponse.getHosts()) {
                    tmpVector.push_back(HostAndPort(range.getMapIp(hp.host(), isFromIpv6), hp.port()));
                }
            ...
            if(isMasterResponse.hasMe()) {
                ...
                auto meHpNew = HostAndPort(range.getMapIp(meHpOld.host(), isFromIpv6), meHpOld.port());
                isMasterResponse.setMe(meHpNew);
            }

            if (isMasterResponse.hasPrimary()) {
                auto primaryHpOld = isMasterResponse.getPrimary();
                auto primaryHpNew = HostAndPort(range.getMapIp(primaryHpOld.host(), isFromIpv6), primaryHpOld.port());
            ...
            if(isMasterResponse.hasHosts()) {
                std::vector<HostAndPort> tmpVector;
                for(auto& hp : isMasterResponse.getHosts()) {
                    tmpVector.push_back(HostAndPort(range.getPublicIp(hp.host(), isFromIpv6), hp.port()));
                }
                isMasterResponse.setHosts(tmpVector);
            }
            ...
            if(isMasterResponse.hasMe()) {
                auto meHpOld = isMasterResponse.getMe();
                auto meHpNew = HostAndPort(range.getPublicIp(meHpOld.host(), isFromIpv6), meHpOld.port());
                isMasterResponse.setMe(meHpNew);
            }

            if (isMasterResponse.hasPrimary()) {
                auto primaryHpOld = isMasterResponse.getPrimary();
                auto primaryHpNew = HostAndPort(range.getPublicIp(primaryHpOld.host(), isFromIpv6), primaryHpOld.port());
                isMasterResponse.setPrimary(primaryHpNew);
            }
        }
