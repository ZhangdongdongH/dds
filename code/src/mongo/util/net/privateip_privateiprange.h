...
namespace mongo {
    ...
    public:
        PrivateIpPrivateIpRange():_lock("privateIpPrivateIpRangeMutex") {}
        ...
        void parsePrivateIp(std::string privateIp, std::map<std::string, std::string> & tmpPrivateIpMap);
        bool parsePrivateIpRange(std::string & range, std::map<uint32_t, IpRange> & tmpRangeMap);
        std::string& getMapIp(const std::string & privateIp) {
            return ipMap[privateIp];
        }


        std::map<std::string, std::string> getIpMap() {
    ...
    private:
        // key is private ip and value is other private ip
