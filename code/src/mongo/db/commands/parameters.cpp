    ...
    virtual void append(OperationContext* txn, BSONObjBuilder& b, const std::string& name) {
        ...
        virtual Status set(const BSONElement& newValueElement) {
            if (newValueElement.type() != BSONType::Bool) {
                return Status(ErrorCodes::BadValue,
                              str::stream() << "Invalid value type for readOnly");
            }
...
public:
    IsImplicitCreateColSetting() : ServerParameter(ServerParameterSet::getGlobal(), "isImplicitCreateCol") {}
...
public:
    ...
    virtual Status set(const BSONElement& newValueElement) {
        ...
        if (!newValueElement.coerce(&newValue) || newValue < 0)
            return Status(ErrorCodes::BadValue,
                          mongoutils::str::stream()
                              << "Invalid value for maxIncomingConnections: " << newValueElement << " and now MaxConnection is " << getGlobalServiceContext()->getServiceEntryPoint()->getMaxNumConnections());
                              //invalid use of incomplete type 'class mongo::ServiceEntryPoint'
		//return a serviceContext
        Status status = getGlobalServiceContext()->getServiceEntryPoint()->setMaxNumConnections(newValue);
    ...
    virtual Status setFromString(const std::string& str) {
        ...
        if (newValue < 0)
            return Status(ErrorCodes::BadValue,
                          mongoutils::str::stream() << "Invalid value for maxIncomingConnections: " << newValue << " and now MaxConnection is " << getGlobalServiceContext()->getServiceEntryPoint()->getMaxNumConnections());
...
public:
    AllowCommandSetting()
        : ServerParameter(ServerParameterSet::getGlobal(), "allowCommands") {}
    ...
    virtual Status set(const BSONElement& newValueElement) {
        try{
			...
			return Status(ErrorCodes::BadValue,
                          mongoutils::str::stream()
                              << "Invalid parameter for allowCommands: "
                              << newValueElement
                              << ", exception: "
                              << msg.what());
        }
    ...
    virtual Status setFromString(const std::string& str) {
        ...
        for(const auto command:allowcommands_temp){
            if (Command::globleDisableCommands.find(command) == Command::globleDisableCommands.end()) {
                return Status(ErrorCodes::BadValue,
...
public:
    ...
    virtual Status set(const BSONElement& newValueElement) {
        ...
        if (!newValueElement.coerce(&newValue) || newValue < 0)
            return Status(ErrorCodes::BadValue,
                          mongoutils::str::stream()
                              << "Invalid value for maxInternalIncomingConnections: " << newValueElement << " and now MaxInternalConnection is " << getGlobalServiceContext()->getServiceEntryPoint()->getMaxNumInternalConnections());
        Status status = getGlobalServiceContext()->getServiceEntryPoint()->setMaxNumInternalConnections(newValue);
        if (!status.isOK())
            return status;
    ...
    virtual Status setFromString(const std::string& str) {
        ...
        if (newValue < 0)
            return Status(ErrorCodes::BadValue,
                          mongoutils::str::stream() << "Invalid value for maxInternalIncomingConnections: " << newValue << " and now MaxInternalConnection is " << getGlobalServiceContext()->getServiceEntryPoint()->getMaxNumInternalConnections());
        status = getGlobalServiceContext()->getServiceEntryPoint()->setMaxNumInternalConnections(newValue);
        if (!status.isOK())
            return status;
        serverGlobalParams.maxInternalConns = newValue;
        return Status::OK();
