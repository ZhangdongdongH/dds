...
struct ServerGlobalParams {
    ...
    int maxInternalConns = DEFAULT_MAX_INTERNAL_CONN;  //Maximum number of simultaneous open internal connections.
    ...
    std::map<std::string, std::string> auditNsFilterMap; // Filter namespace and op that need to be audited, map format.
    std::map<std::string, std::map<std::string, bool>> auditOpNsFilterMap; 
    int  auditOpFilter = 0;      // Bitwise or result of ops that need to be audited, parsed from auditOpFilterStr.
    bool auditAuthSuccess = true;   // True if audit authorization success requests.
    bool isImplicitCreateCol = false;  //True , insert a item can create collection auto, False, a collection is created before inert
