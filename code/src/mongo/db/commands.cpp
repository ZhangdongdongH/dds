...
bool Command::checkIfDisableCommands(const std::string& cmdname) {
    ...
    if (Command::globleDisableCommands.find(cmdname) != Command::globleDisableCommands.end() &&
        find(serverGlobalParams.allowCommands.begin(), serverGlobalParams.allowCommands.end(), cmdname) 
...
static bool _isDisabledCommandsAndPameterForCustomer(const Command* c, const std::string& cmdname,
    ...
    if (dbname == "admin" || dbname == "config") {
        ...
        if (fullNs != "admin.system.users" && fullNs != "admin.system.roles" && fullNs != "admin.system.version") {
...
bool CommandHelpers::uassertShouldAttemptParse(OperationContext* opCtx,
    ...
    try {
		...
		if(false == result){
			...
			uassert(ErrorCodes::Unauthorized,
							str::stream() << command->getName() << " No storage space to execute: " << command->getName(),
        ...
        if (checkAuthorizationImplPreParse(opCtx, c, request)) {
			...
			if (!status.isOK()) {
				...
				if (status == ErrorCodes::CommandNotFound) {
					uasserted(ErrorCodes::Unauthorized, str::stream() << "no this command" << cmdToLog.toString());
				} else {
					uasserted(ErrorCodes::Unauthorized, str::stream() << "not  authorized on " << dbname << " to execute command " << cmdToLog.toString());
				}
			}
        }
...
bool Command::_checkWriteAllow(OperationContext* txn,
