...
DbResponse receivedQuery(OperationContext* opCtx,
    ...
    try {
        ...
        if (!AuthorizationSession::get(opCtx->getClient())->shouldAllowLocalhost()
            &&  (AuthorizationSession::get(opCtx->getClient())->isAuthWithCustomerOrNoAuthUser() || opCtx->isCustomerTxn())
            ...
            if (nss == std::string("admin.system.users")) {
                ...
                BSONObj filterdbname = BSON(AuthorizationManager::USER_DB_FIELD_NAME << NE << "admin");
        ...
        if (nsString == std::string("admin.system.users")) {
            for (const auto& obj : insertOp.getDocuments()) {
                ...
                if (status1.isOK() && status2.isOK()
                    ...
                    Status status(ErrorCodes::Unauthorized, "userName: " + userName + " conflicted with builtin users within admin db.");
                    uassertStatusOK(status);
                }
            }
        }
        ...
        if (nsString == std::string("admin.system.roles")) {
            for (const auto& obj : insertOp.getDocuments()) {
                ...
                if (status1.isOK() && status2.isOK()
                    ...
                    Status status(ErrorCodes::Unauthorized, "roleName: " + roleName + " conflicted with builtin roles within admin db.");
                    uassertStatusOK(status);
                }
            }
        }
