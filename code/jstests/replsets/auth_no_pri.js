...
(function() {
    ...
    master.getDB("admin").createUser({user: "admin", pwd: "Github@12", roles: ["root"], "passwordDigestor" : "server"}, {w: NODE_COUNT});

    // Can authenticate replset connection when whole set is up.
    var conn = new Mongo(rs.getURL());
    assert(conn.getDB('admin').auth('admin', 'Github@12'));
    ...
    assert(conn2.getDB('admin').auth({user: 'admin', pwd: 'Github@12', mechanism: "SCRAM-SHA-1"}));
