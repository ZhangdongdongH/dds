...
assert.commandFailed(
    mydb.runCommand({createUser: 1, user: "spencer", pwd: "Github@12", readOnly: true, "digestPassword" : true}));

// V1 user document; insert should fail.
assert.commandFailed(
    mydb.runCommand({createUser: 1, user: "spencer", userSource: "test2", roles: ["dbAdmin"], "digestPassword" : true}));

// Valid V2 user document; insert should succeed.
assert.commandWorked(mydb.runCommand({createUser: "spencer", pwd: "Github@12", roles: ["dbAdmin"], "digestPassword" : true}));
...
assert.commandFailed(mydb.runCommand({createUser: "bob", pwd: "Github@12", roles: ["fakeRole123"], "digestPassword" : true}));
...
assert.commandFailed(mydb.runCommand({updateUser: 'spencer', pwd: "", "passwordDigestor" : "server"}));
