...
function createUsers(conn) {
    var adminDb = conn.getDB(adminDbName);
    var firstDb = conn.getDB(firstDbName);
    adminDb.createUser({user: "admin", pwd: "Password@a1b", roles: ["__system"], "passwordDigestor" : "server"});
    ...
    assert.commandWorked(adminDb.runCommand(
        {createUser: testUser, pwd: "Password@a1b", roles: [{role: testRole, db: adminDbName}], "digestPassword" : true}));
