    ...
    if (cmdObj.hasField("pwd")) {
        ...
        if (parsedArgs->digestPassword) {
            ...
            if(checkPasswordSameOrReverseWithUsername(parsedArgs->password, userName)) {
                return Status(ErrorCodes::BadValue, "User passwords must not same with username or username's reverse.");
            }
            parsedArgs->hashedPassword = mongo::createPasswordDigest(userName, parsedArgs->password);
        } else {
...
bool isInWeakPasswordDict(std::string password) {
    std::string weakPasswordDict[] = {"Admin@","Password@a1b3","Admin@123",
                                          "admin123","admin123!","Admin123!","Administrator","Password",
