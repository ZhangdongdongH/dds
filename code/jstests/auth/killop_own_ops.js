...
(function() {
    ...
    function runTest(m, failPointName) {
        ...
        db.createUser({user: 'reader', pwd: 'Password@a1b', roles: [{db: 'foo', role: 'read'}], "passwordDigestor" : "server"});
        db.createUser(
            {user: 'otherReader', pwd: 'Password@a1b', roles: [{db: 'foo', role: 'read'}], "passwordDigestor" : "server"});
        admin.createRole({
            role: 'opAdmin',
            roles: [],
            privileges: [{resource: {cluster: true}, actions: ['inprog', 'killop']}]
        });
        db.createUser({user: 'opAdmin', pwd: 'Password@a1b', roles: [{role: 'opAdmin', db: 'admin'}], "passwordDigestor" : "server"});
