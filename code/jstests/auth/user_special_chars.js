"use strict";
// Test creating and authenticating users with special characters.

(function() {
    var conn = MongoRunner.runMongod({auth: ''});

    var adminDB = conn.getDB('admin');
    adminDB.createUser({user: 'admin', pwd: 'Password@a1b', roles: jsTest.adminUserRoles, "passwordDigestor" : "server"});

    var testUserSpecialCharacters = function() {

        // Create a user with special characters, make sure it can auth.
        assert(adminDB.auth('admin', 'Password@a1b'));
        adminDB.createUser(
            {user: '~`!@#$%^&*()-_+={}[]||;:",.//><', pwd: 'Password@a1b', roles: jsTest.adminUserRoles, "passwordDigestor" : "server"});
        assert(adminDB.logout());

        assert(adminDB.auth({user: '~`!@#$%^&*()-_+={}[]||;:",.//><', pwd: 'Password@a1b'}));
        assert(adminDB.logout());
    };
    testUserSpecialCharacters();

    var testUserAndDatabaseAtSymbolConflation = function() {
        // Create a pair of users and databases such that their string representations are
        // identical.
        assert(adminDB.auth('admin', 'Password@a1b'));

        var bcDB = conn.getDB('b@c');
        bcDB.createUser({user: 'a', pwd: 'Password@a1b2', roles: [{role: 'readWrite', db: 'b@c'}], "passwordDigestor" : "server"});

        var cDB = conn.getDB('c');
        cDB.createUser({user: 'a@b', pwd: 'Password@a1b1', roles: [{role: 'readWrite', db: 'c'}], "passwordDigestor" : "server"});

        assert(adminDB.logout());

        // Ensure they cannot authenticate to the wrong database.
        assert(!bcDB.auth('a@b', 'Password@a1b1'));
        assert(!bcDB.auth('a@b', 'Password@a1b2'));
        assert(!cDB.auth('a', 'Password@a1b1'));
        assert(!cDB.auth('a', 'Password@a1b2'));

        // Ensure that they can both successfully authenticate to their correct database.
        assert(cDB.auth('a@b', 'Password@a1b1'));
        assert.writeOK(cDB.col.insert({data: 1}));
        assert.writeError(bcDB.col.insert({data: 2}));
        assert(cDB.logout());

        assert(bcDB.auth('a', 'Password@a1b2'));
        assert.writeOK(bcDB.col.insert({data: 3}));
        assert.writeError(cDB.col.insert({data: 4}));
        assert(bcDB.logout());

        // Ensure that the user cache permits both users to log in at the same time
        assert(cDB.auth('a@b', 'Password@a1b1'));
        assert(bcDB.auth('a', 'Password@a1b2'));
        assert(cDB.logout());
        assert(bcDB.logout());

        assert(bcDB.auth('a', 'Password@a1b2'));
        assert(cDB.auth('a@b', 'Password@a1b1'));
        assert(cDB.logout());
        assert(bcDB.logout());
    };
    testUserAndDatabaseAtSymbolConflation();

    MongoRunner.stopMongod(conn);
})();
