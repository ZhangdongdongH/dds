...
function copydbBetweenClustersTest(configObj) {
    ...
    if (configObj.isSourceUsingAuth) {
        ...
        source.conn.getDB("admin").createUser(
            {user: "admin", pwd: "Github@12", roles: ["root"], "passwordDigestor" : "server"});
        source.conn.getDB("admin").auth("admin", "Github@12");
        ...
        source.conn.getDB(baseName).createUser({user: "monitor", pwd: "Github@125", roles: ["dbOwner"], "passwordDigestor" : "server"});
    ...
    if (configObj.isTargetUsingAuth) {
        ...
        target.conn.getDB("admin").auth("admin", "Github@12");
    }
    ...
    if (configObj.isSourceUsingAuth) {
        ...
        assert.commandWorked(target.conn.getDB(baseName).copyDatabase(
            baseName, baseName, source.connString, "monitor", "Github@125"));
    } else {
