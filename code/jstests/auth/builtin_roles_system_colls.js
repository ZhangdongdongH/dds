...
function testUserAdminAnyDatabaseSystemCollIndexing(adminDB) {
    ...
    adminDB.createUser({user: "monitor", pwd: "Password@a1b", roles: ["userAdminAnyDatabase"], "passwordDigestor" : "server"});
    adminDB.logout();
...
function testBackupSystemCollStats(adminDB) {
    adminDB.auth("admin", "Password@a1b");
    adminDB.createUser({user: "backupuser", pwd: "Password@a1b", roles: ["backup"], "passwordDigestor" : "server"});
