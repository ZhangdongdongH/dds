...
setupConn.getDB('foo').createUser({user: 'foo', pwd: 'Password@a1b', roles: jsTest.basicUserRoles, "passwordDigestor" : "server"},
    {w: NUM_NODES});
setupConn.getDB('foo').logout();
setupConn.getDB('bar').createUser({user: 'bar', pwd: 'Password@a1b', roles: jsTest.basicUserRoles, "passwordDigestor" : "server"},
    {w: NUM_NODES});
