...
Status addBaseServerOptions(moe::OptionSection* options) {
    ...
    portInfoBuilder << "specify port number - " << ServerGlobalParams::DefaultDBPort
                    << " by default";
    maxInternalConnInfoBuilder << "max number of simultaneous internal connections - " << DEFAULT_MAX_INTERNAL_CONN
                    << " by default";
    ...
    options->addOptionChaining(
        "net.maxInternalIncomingConnections", "maxInternalConns", moe::Int, maxInternalConnInfoBuilder.str().c_str());
    ...
    options->addOptionChaining("security.readOnly",
                               "readOnly",
                               moe::Switch,
                               "enable readOnly");

    options->addOptionChaining("auditLog.path",
                               "",
                               moe::String,
    ...
    options->addOptionChaining("auditLog.format",
                               "",
                               moe::String,
                               "Desired audit log format, JSON or HWDDS")
                               .setSources(moe::SourceYAMLConfig);
    ...
    options->addOptionChaining("auditLog.authSuccess", "", moe::Bool, "enable audit authorization success")
                               .setSources(moe::SourceYAMLConfig);
    options
        ->addOptionChaining("auditLog.nsFilter",
                            "",
                            moe::StringMap,
                            "specify namespace and operations to audit")
                            .setSources(moe::SourceYAMLConfig);
...
Status storeBaseOptions(const moe::Environment& params) {
    ...
    if (params.count("systemLog.traceAllExceptions")) {
     ...
     if (params.count("net.maxInternalIncomingConnections")) {
        serverGlobalParams.maxInternalConns = params["net.maxInternalIncomingConnections"].as<int>();
    ...
    if (params.count("auditLog.format")) {
        ...
        if (serverGlobalParams.auditLogFormat != "JSON" && serverGlobalParams.auditLogFormat != "HWDDS") {
            StringBuilder sb;
...
bool parseAuditNsFilter(const std::map<std::string, std::string>& auditNsFilterMap) {
    ...
    for (auto& nsFilterIt : auditNsFilterMap) {
        if (std::find(logger::auditOpList.begin(), logger::auditOpList.end(), nsFilterIt.first) == logger::auditOpList.end()) {            
            return false;
        }
    }
